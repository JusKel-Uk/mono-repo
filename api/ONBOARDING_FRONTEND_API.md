...
| After admin picks up review | `GET` | `/onboarding/applications/current` | `status: 2` (InReview) |
| After admin publish | `GET` | `/onboarding/applications/current` | `status: 3` (Published), `publishedAt` set (no public publish endpoint yet) |
...
**Review lifecycle (admin — later):** There is no SME-facing endpoint to advance review status yet. Admins will transition `Submitted → InReview → Published` in a future slice. Until then, set `status` in the database: `2` = InReview, `3` = Published (with `publishedAt` when published).
